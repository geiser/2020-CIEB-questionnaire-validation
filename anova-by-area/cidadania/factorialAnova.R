...
ggPlotAoV <- function(data, x, y, color = c(), aov, pwc, linetype = color, by = c(), addParam = c(), step.increase = 0.005) {
  pwc <- tryCatch(add_xy_position(pwc, x = x, step.increase = step.increase), error = function(e) NULL)
...
dat <- read.csv("anova-by-area/cidadania/data.csv")[,c("ID", "area.de.conhecimento", "cidadania" )]
