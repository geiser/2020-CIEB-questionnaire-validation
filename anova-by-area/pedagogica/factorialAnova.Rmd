---
title: "ANOVA `pedagogica` ~ `area.de.conhecimento`"
...
dat <- read.csv("data.csv")[,c("ID", "area.de.conhecimento", "pedagogica" )]
...
```{r echo=FALSE}
kdf <- add_significance(normality_test_at(group_by(sdat, `area.de.conhecimento`), "pedagogica"))
kdf$p <- round(kdf$p, 4)
kdf$p[which(kdf$p < 0.0001)] <- '< 0.0001'
kable(kdf, digits = 4)
```
...
```{r echo=FALSE}
kdf <- add_significance(levene_test(sdat, `pedagogica` ~ `area.de.conhecimento`))
kdf$p <- round(kdf$p, 4)
kdf$p[which(kdf$p < 0.0001)] <- '< 0.0001'
kable(kdf, digits = 4)
```
