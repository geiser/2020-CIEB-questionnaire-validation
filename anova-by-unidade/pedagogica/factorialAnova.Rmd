---
title: "ANOVA `pedagogica` ~ `unidade`"
...
dat <- read.csv("data.csv")[,c("ID", "unidade", "pedagogica" )]
...
```{r echo=FALSE}
kdf <- add_significance(normality_test_at(group_by(sdat, `unidade`), "pedagogica"))
kdf$p <- round(kdf$p, 4)
kdf$p[which(kdf$p < 0.0001)] <- '< 0.0001'
kable(kdf, digits = 4)
```
...
```{r echo=FALSE}
kdf <- add_significance(levene_test(sdat, `pedagogica` ~ `unidade`))
kdf$p <- round(kdf$p, 4)
kdf$p[which(kdf$p < 0.0001)] <- '< 0.0001'
kable(kdf, digits = 4)
```
